...
public:
	inline auto& get() const { return *this->_object; }
	inline T& operator()() const { return *this->_object; }
};
...
struct get_accessor {
	using type = typename std::conditional<
		std::is_same<Accessor, ac::get>::value, get_access_property<T>,
		typename std::conditional<std::is_same<Accessor, ac::set>::value
		, set_access_property<T>, nothing
		>::type
	>::type;
};
